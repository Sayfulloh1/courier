...
class CourierRepositoryImpl extends CourierRepository {
  ...
  @override
  Future<Either<Exception, Map<String,dynamic>>> pickDeliver(
      String id) async {
  ...
  Future<Either<Exception, List<Map<String, dynamic>>>> getAllCompletedOrders() async {
    ...
    String url = 'https://ulab-market-v2-n6zf.onrender.com/api/order/courier/myorders?limit=1000&page=1';
    try {
      Dio dio = Dio();
      // Make GET request with bearer token in headers
      Response response = await dio.get(
      ...
      if (response.statusCode == 200) {
        ...
        List<Map<String, dynamic>> result = List<Map<String, dynamic>>.from(data['data']);
        return Right(result);
      } else {
        return Left(Exception('Failed to fetch orders. Status code: ${response.statusCode}'));
      }
    } catch (error) {
      print('error occurred while fetching orders');
      return Left(Exception(error.toString()));
